...
from keras_frcnn import config, data_generators, FixedBatchNormalization
...
data_gen_train = data_generators.get_anchor_gt(train_imgs, classes_count, C, nn.get_img_output_length, K.backend(),
                                               mode='train')
data_gen_val = data_generators.get_anchor_gt(val_imgs, classes_count, C, nn.get_img_output_length, K.backend(),
                                             mode='val')
...
for epoch_num in range(num_epochs):
    ...
    while True:
        try:
            ...
            R = roi_helpers.rpn_to_roi(P_rpn[0], P_rpn[1], C, K.backend(), use_regr=True, overlap_thresh=0.7,
                                       max_boxes=300)
            ...
            if iter_num == epoch_length:
                ...
                if curr_loss < best_loss:
                    if C.verbose:
                        print(f'Total loss decreased from {best_loss:.3f} to {curr_loss:.3f}, saving weights to {C.model_path}')
                    best_loss = curr_loss
